...
void clearScreen() {
    system("cls");
    textColor(WHITE, _BLACK); // Redefinindo as cores após limpar a tela
}
...
void listUsers() {
    ...
    while (getch() != 27); // Espera até o usuário pressionar ESC
}
//---------------------------------------------------------------------------------------------
void menuLogin() {
    char username[15];
    ...
    while (!valid) {
        ...
        int i = 0;
        char ch;
        while ((ch = getch()) != '\r' && i < 29) { // Lê até Enter
            if (ch == 8 && i > 0) { // Backspace
                i--;
                printf("\b \b"); // Limpa o último caractere na tela
            } else {
                password[i++] = ch;
